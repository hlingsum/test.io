...
3. Parameters for ESTARFM.pro
 w=25                 ;set the haif window size, if 25, the window size is 25*2+1=51
num_similar_pixel=20         ;set number of similar pixels
 num_class=4                    ;set the estimated number of classes
 DN_min=0                       ;set the valid range of DN value of the image,If byte, 0 and 255
 DN_max=10000
 background=-9999           ;the value of background and missng pixels in both MODIS and Landsat images
 patch_long=500                 ;set the size of block,if process whole TM scene, 500 is recommended  
 temp_file='G:\temp'            ;set the temporary file folder

(1)w=25         
Set the window size (half),  if 25, the window size is 25*2+1=51;
(2) num_similar_pixel=20         ;set number of similar pixels, the code runs faster with a smaller value but too small value may reduce the accuracy, 20 is recommended
(3) num_class=4             
...
The blended result will be saved in the folder of the original images automatically. The names are the original prediction Modis image name followed by '_ESTARFM_FAST'
